 ...
 * @author Baltazar
 */
public class Compras_Calzado1 extends javax.swing.JFrame {
    ...
    String sql_tabla = "Select Id_Producto,Nombre_Producto,Talla,Marca,Color from producto";
    int cont = 0;
    ...
    public Compras_Calzado1(ModeloSession mss) {
        initComponents();
        this.mss=mss;
        ...
        Ajuste_Columna_Tabla(JT_Compra, 5, 80, 80, 80);
    ...
    public void llenarTabla_compra() {
        ...
        dt2.addRow(new Object[]{Id_Producto, Nombre_Producto, Talla_Producto, Marca_Producto, Color_Producto, TFRI_Cantidad.getText(), String.valueOf(df.format(Precio))});
        total = 0;

        for (int i = 0; i < JT_Compra.getRowCount(); i++) {
            canti = Integer.parseInt(JT_Compra.getValueAt(i, 5).toString());
            prec = Float.parseFloat(JT_Compra.getValueAt(i, 6).toString());
    ...
    public void modificarTabla_compra(int i) {
        ...
        int cant = Integer.parseInt(TFRI_Cantidad.getText().toString()) + Integer.parseInt(JT_Compra.getValueAt(i, 5).toString());
        int canti;
        float prec;

        JT_Compra.setValueAt(cant, i, 5);
        JT_Compra.setValueAt(df.format(precio), i, 6);
        total = 0;
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Administración de Compra Calzado");
        ...
        JT_Compra.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Id", "Nombre", "Talla", "Marca", "Color", "Cantidad", "Precio"
            }
        ));
        ...
        jButton7.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Iconos/refresh.png"))); // NOI18N
    ...
    private void JT_TablaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_JT_TablaMouseClicked
        ...
        Talla_Producto = JT_Tabla.getValueAt(filaSeleccionada, 2).toString();
        Marca_Producto = JT_Tabla.getValueAt(filaSeleccionada, 3).toString();
        Color_Producto = JT_Tabla.getValueAt(filaSeleccionada, 4).toString();
...
//        if (msm.ms_pregunta("Confirme: ¿Desea realizar esta compra?") == 0) {
        if (0 != validar2()) {
            ...
            cb.insertarAccion(mss.getNombre(),"Realizo compra de calzado");

            for (int i = 0; i < JT_Compra.getRowCount(); i++) {
                Id_Producto = Integer.parseInt(JT_Compra.getValueAt(i, 0).toString());
                Cantidad_Compa = Integer.parseInt(JT_Compra.getValueAt(i, 5).toString());
                Precio_Compra = Float.parseFloat(JT_Compra.getValueAt(i, 6).toString());
               ...
               // cb.insertarAccion(mss.getNombre(),"Realizo compra de calzado");
                ...
                cb.insertarAccion(mss.getNombre(),"Modificó inventario");
            }
            Datos.cerrarConexion();
            limpiar();
            limpiar_Tabla();
...
//        Llenar(sql_tabla + " Where Id_Departamento=" + Numero_Departamento + " AND Id_Proveedor="+JC_Proveedor.getSelectedItem().toString()+"", "producto");
    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton7ActionPerformed
        int cantidad;
        float precio, total;
        cantidad = Integer.parseInt(JT_Compra.getValueAt(JT_Compra.getSelectedRow(), 5).toString());
        precio = Float.parseFloat(JT_Compra.getValueAt(JT_Compra.getSelectedRow(), 6).toString());
...
//            public void run() {
//                new Compras_Calzado1().setVisible(true);
//            }
//        });
//    }
